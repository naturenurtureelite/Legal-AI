...
for i in range(0,50):
	...
	for j in range(0,len(count)):
		for k in range(0,len(count)):
			...
			#g.add_edge(k,j,weight=value)
	#h,a1=nx.hits(g)
	#print(a[0],a[1],h[0],h[1])
	model = DeepWalk(walk_length=200, dimensions=64, window_size=5)
	model.fit(g)
	...
	#			print(a[j],a[k])
				...
				#emb1=list(embedding)
				#emb2=list(v10)
				value=cosinesimilarity(embedding[j],embedding[k])
				if value>=0.0:
