...
#a=["ChineseGist"]
#a=["BERT","SummaRuNNer/AttnRNN_F7_avr","SummaRuNNer/CNN_RNN_F7_avr","SummaRuNNer/RNN_RNN_F7_avr","ChineseGist"]
...
for i in range(0,50):
	...
	for l in range(0,len(components)):
		for k in range(0,len(components[l])):
			...
			ttt.append([v10[components[l][k]],len(parsed)])
			#file5.write(vline[components[l][k]]+"."+"\t"+vrhetoric[components[l][k]]+"\n")
		ttt.sort(key = sortSecond,reverse=False)
		

		
		for k in range(0,len(ttt)):
			no_of_words =no_of_words+ttt[k][1]
